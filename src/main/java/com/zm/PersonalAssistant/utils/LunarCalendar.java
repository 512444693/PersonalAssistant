...
public class LunarCalendar implements Comparable<LunarCalendar> {
    ...
    public int getMonth() {
        return chineseCalendar.get(ChineseCalendar.MONTH) + 1;
    }

    public int getChineseMonth() {
        return Math.abs(chineseCalendar.get(ChineseCalendar.CHINESE_MONTH));
    }

    public int getDate() {
        return chineseCalendar.get(ChineseCalendar.DATE);
    }

    public int getChineseDate() {
        return chineseCalendar.get(ChineseCalendar.CHINESE_DATE);
    }

    public void addDate(int numOfDays) {
        chineseCalendar.add(ChineseCalendar.DATE, numOfDays);
    }

    //不是闰月回0， 下月为闰月回1， 闰月回2
    public int isLeapMonth() {
        if(chineseCalendar.get(ChineseCalendar.CHINESE_MONTH) < 0){
            return 2;
        }else{
            ChineseCalendar cc = new ChineseCalendar(this.getChineseYear(), this.getChineseMonth(), this.getChineseDate());
            cc.add(ChineseCalendar.CHINESE_MONTH, 1);
            if(cc.get(ChineseCalendar.CHINESE_MONTH) < 0){
                return 1;
            }else{
                return 0;
            }
 ...
 */
final class ChineseCalendar extends GregorianCalendar {
